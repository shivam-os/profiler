...
export const getSingleProfile = async (id) => {
  const response = await api.get(`/profiles/${id}`, setAuthHeader());
  return response;
};

//Post request to create a new profile
export const createProfile = async (profileData) => {
  const response = await api.post("/profiles", profileData, setAuthHeader());
  return response;
};

//DELETE request to delete a profile
export const deleteProfile = async (profileId) => {
  const response = await api.delete(`/profiles/${profileId}`, setAuthHeader());
  return response;
};

//DELETE request to delete a link with given id
export const deleteLink = async (linkId) => {
  const response = await api.delete(`/profiles/links/${linkId}`, setAuthHeader());
  return response;
}

//PUT request to update a profile with given id
export const updateProfile = async (profileId, profileData) => {
  const response = await api.put(`/profiles/${profileId}`, profileData, setAuthHeader());
  return response
}
